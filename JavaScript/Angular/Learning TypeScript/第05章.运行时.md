...
第 11 行中，指定子类对象构造函数 (d) 的原型。若父类对象构造函数为空时，使用 Object.create()方法创建一个新对象，使用现有的对象来提供新创建的对象的 \__proto__ (即内部[[Prototype]]属性），返回一个原型为空的对象( {} )；反之，则将父类对象构造函数 (b) 的原型赋值给 __ 对象构造函数的原型，返回 new __() 的实例。对于该实例，其原型 [[prototype]] 是父类对象构造函数 (b) 的原型，其 constructor 属性指向子类对象构造函数 (b)。
