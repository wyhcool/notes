...
典型的模块是一个包含 exports 对象属性定义的文件，这些属性可以是任意类型的数据，比如字符串、对象或函数。
...
使用 module.exports 可以对外提供单个变量、函数或对象。如果创建了一个既有 exports 又有 module.exports 的模块，那它会返回 module.exports，而 exports 会被忽略。

注意⚠️：最终在程序中导出的是 module.exports，而 exports 只是对 module.exports 的一个全局引用，被定义为一个可以添加属性的空对象。要维持 module.exports 和 exports 之间的引用关系：
```javascript
